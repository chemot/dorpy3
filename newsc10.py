...
ririn = LINE("EvVGEzt3C0nkr0RpHoea.UHbbg4pHBjT5VwCYtXmXsG.kcJ7VbYwvFOjWeXtsVd3VaQ5LrjD5Z4L1a6l8ptMo6M=")
#ririn = LINE("")
...
# Pembatas Script #
        ...
        if op.type == 26:
            ...
            if wait["likeOn"] == True:
                ...
                hasil = line.getHomeProfile(mid=mintaLike)
                st = hasil['result']['feeds']
                for i in range(len(st)):
                    test = st[i]
                    result = test['post']['postInfo']['postId']
                    ririn.likePost(mid=mintaLike, postId=result, likeType=random.choice(typel))
                    ririn.createComment(str(sender), str(result), 'Auto Like by HelloWorld & Tanysyz')
                ririn.sendMessage(to, 'Like & Comment '+str(len(st))+' Post dari {}'.format(line.getContact(mintaLike).displayName))
